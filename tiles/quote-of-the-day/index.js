(() => {
  ...
  function setup(m){
    ...
    daily.watch(m,()=>{render();daily.feed().then(data=>{if(disposed)return;live=(data?.quotes||[]).filter(q=>window.TeacherTilesLive.validQuote(q)&&q.source?.startsWith('https://en.wikiquote.org/wiki/'));render();});});
