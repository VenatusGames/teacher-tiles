...
window.createStickerPicker=function({panel,packs,search,clear,status,shell,bindDrag,owns,require:requestAccess,entitlement,place}){
  ...
  function render(){
    ...
    for(const pack of displayed){
      ...
      toggle.type='button';toggle.className='sticker-picker-pack-toggle';name.textContent=pack.name;total.textContent=items.length;total.className='sticker-picker-pack-count';chevron.innerHTML='<svg viewBox="0 0 20 20" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m5 7.5 5 5 5-5"/></svg>';chevron.className='sticker-picker-chevron';chevron.setAttribute('aria-hidden','true');toggle.append(name,total,chevron);heading.append(toggle);
      ...
      if(locked){
        ...
        const lock=document.createElement('button');lock.type='button';lock.className='sticker-picker-pack-lock';lock.dataset.entitlement=items[0].product;lock.setAttribute('aria-label','Unlock '+pack.name+' sticker pack');lock.innerHTML='<span><img src="assets/ui/lock.svg?v=20260927-gold" alt="">Unlock pack</span>';lock.addEventListener('click',()=>requestAccess(lock));section.append(lock);
        const badge=document.createElement('span');badge.className='sticker-picker-pack-lock-mark';badge.innerHTML='<img src="assets/ui/lock.svg?v=20260927-gold" alt="">';badge.setAttribute('aria-hidden','true');toggle.insertBefore(badge,total);
      }
