...
window.createStickerPicker=function({panel,packs,search,clear,status,shell,bindDrag,owns,require:requestAccess,entitlement,place}){
  ...
  function render(){
    ...
    const seen=new Set();
    const mostUsed=catalog.flatMap(pack=>pack.items).filter(item=>{if(seen.has(item.key)||!usage[item.key]||!owns(item.product))return false;seen.add(item.key);return true}).sort((a,b)=>usage[b.key]-usage[a.key]||a.name.localeCompare(b.name)).slice(0,10);
    ...
    for(const pack of displayed){
      ...
      for(const item of items){
        ...
        const button=item.source.cloneNode(true);delete button.dataset.stickerDragReady;button.removeAttribute('id');button.classList.remove('is-search-hidden','is-dragging','is-cosmetic-locked');button.dataset.entitlement=item.product;button.querySelectorAll('.subscription-crown').forEach(el=>el.remove());button.setAttribute('aria-label',`${owns(item.product)?'Add':'Unlock'} ${item.name}`);button.title=item.name;button.tabIndex=0;
      ...
      const locked=items.length>0&&items.every(item=>!owns(item.product));
      if(locked){
