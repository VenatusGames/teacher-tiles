(()=>{
  ...
  function setup(m,{catalog,owned,apply}){
    ...
    function position(){if(!m.isConnected){close();return;}const r=button.getBoundingClientRect(),d=drawer.getBoundingClientRect();drawer.style.left=Math.max(8,Math.min(innerWidth-d.width-8,r.left))+'px';drawer.style.top=Math.max(8,Math.min(innerHeight-d.height-8,r.top-d.height-8))+'px';frame=requestAnimationFrame(position);}
    function render(){
      ...
      for(const skin of [null,...catalog.filter(s=>s.tileType===m.dataset.type)]){
        const id=skin?.id||'',locked=skin&&!owned(skin),selected=(m.dataset.tileSkin||'')===id,card=document.createElement('button');card.type='button';card.className='tile-skins-choice';card.disabled=!!locked;card.setAttribute('aria-pressed',String(selected));card.title=skin?.description||'Original tile appearance';
        ...
        const name=document.createElement('strong'),status=document.createElement('small');name.textContent=skin?.name||'Default';status.textContent=locked?'🔒 Locked · Shop':selected?'✓ Applied':'Apply skin';card.append(preview,name,status);card.onclick=()=>{if(skin&&!owned(skin))return;close();const next=apply(m,id);next?.querySelector(':scope > .tile-skins-toggle')?.focus();};grid.append(card);
      }
    }
