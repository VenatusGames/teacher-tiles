  ...
  function setup(m,{catalog,owned,apply}){
    ...
    function render(){
      ...
      for(const skin of [null,...catalog.filter(s=>s.tileType===m.dataset.type)]){
        ...
        const name=document.createElement('strong'),status=document.createElement('small');name.textContent=skin?.name||'Default';status.textContent=locked?'🔒 Locked · Shop':selected?'✓ Applied':'Apply Skin';card.append(preview,name,status);card.onclick=()=>{if(skin&&!owned(skin)){close();window.TeacherTilesShop?.openPage('tile-skins');return;}close();const next=apply(m,id);next?.querySelector(':scope > .tile-skins-toggle')?.focus();};grid.append(card);if(skin)markSubscriptionAccess(card,skin.productId);
      }
    }
