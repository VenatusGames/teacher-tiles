(() => {
  ...
  function setup(m){
    ...
    function paintFormat(input){const f=formats[input.dataset.cell]||{},size=f.size||({h1:28,h2:22,h3:18}[f.block])||12;Object.assign(input.style,{fontFamily:f.font||'inherit',fontSize:size+'px',fontWeight:f.bold||/^h[123]$/.test(f.block)?'700':'400',fontStyle:f.italic||f.block==='blockquote'?'italic':'normal',textDecoration:[f.underline?'underline':'',f.strike?'line-through':''].filter(Boolean).join(' ')||'none',textAlign:f.align||'left',color:f.color||'inherit',backgroundColor:f.highlight||'transparent',whiteSpace:f.wrap?'pre-wrap':'pre',paddingTop:pictures[input.dataset.cell]?Math.max(4,(rowHeights[coords(input.dataset.cell)[0]]||29)-27)+'px':'4px'});input.wrap=f.wrap?'soft':'off';input.spellcheck=spellcheck;
      let image=input.parentElement.querySelector('.sheet-cell-image');if(pictures[input.dataset.cell]){if(!image){image=document.createElement('img');image.className='sheet-cell-image';image.alt='Cell image';input.before(image)}if(image.src!==pictures[input.dataset.cell])image.src=pictures[input.dataset.cell]}else image?.remove();
    }
    ...
    command('Wrap cell text','Wrap',()=>apply('wrap',!(formats[selected]||{}).wrap),'wrap');
    ...
    const upload=document.createElement('input');upload.type='file';upload.accept='image/png,image/jpeg,image/webp';upload.hidden=true;upload.setAttribute('data-skip-board-save','');formatBar.append(upload);command('Insert cell image','Image',()=>{if(selected)upload.click()});command('Remove cell image','−Image',()=>{if(!selected)return;remember();for(const ref of selection())delete pictures[ref];changed()});
    ...
    function refresh(){const cache=new Map(),selectedCells=new Set(selection());grid.querySelectorAll('textarea').forEach(input=>{if(input!==document.activeElement)input.value=display(input.dataset.cell,cache);input.classList.toggle('has-error',String(input.value).startsWith('#'));input.classList.toggle('is-selected',selectedCells.has(input.dataset.cell));paintFormat(input)});label.textContent=rangeAnchor&&rangeAnchor!==selected?rangeAnchor+':'+selected:selected;syncFormatToolbar();status.textContent=`${rows} rows × ${cols} columns`}
