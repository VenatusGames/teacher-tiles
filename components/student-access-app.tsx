...
function StudentHistory({ token, student }: { token: string; student: Student }) {
  ...
  return <div className="history-browser">
    ...
    {loading ? <p className="history-feedback">Loading check-ins…</p> : <>
      ...
      <div className="history-records">{entries.map(entry => <details className="history-record" key={`${entry.studentId}:${entry.id}`}><summary><span><strong>{formatDate(entry.createdAt)}</strong><small>{entry.items.length} answer{entry.items.length === 1 ? '' : 's'} · Select to view</small></span></summary><dl>{entry.items.map((item, index) => <div key={index}><dt>{item.question}</dt><dd><HistoryAnswer item={item} />{item.imageKey && <span>{item.answer}</span>}</dd></div>)}</dl></details>)}</div>
    </>}
