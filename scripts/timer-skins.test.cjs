...
(async()=>{
  ...
  try{
    ...
    await page.evaluate(()=>{const m=workspace.querySelector('.module');applyTileSkinToModule(m,'');});
    assert.equal(await tile.getAttribute('data-tile-skin'),null,'Default removes the skin selection');
    const defaultFill=await tile.locator('.shape-fill').evaluate(el=>({gradient:getComputedStyle(el).backgroundImage,overlay:getComputedStyle(el,'::before').content}));
    assert(defaultFill.gradient.includes('radial-gradient')&&defaultFill.gradient.includes('conic-gradient'),'default keeps its original shaded progress fill');assert.equal(defaultFill.overlay,'none','default has no Solid material overlay');
