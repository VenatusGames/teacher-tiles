...
(async()=>{
  ...
  try{
    ...
    assert((await tile.locator('.shape-fill').evaluate(el=>getComputedStyle(el).backgroundImage)).includes('rgb(220, 231, 244)'),'Solid uses the empty color for the unfilled area');
    await page.evaluate(()=>{TeacherTilesBoard.clear();for(const [i,skin] of ['timer-freestanding','timer-solid','timer-liquid'].entries()){const m=createModule('timer',400,200,{record:false,tileSkin:skin});m.style.left=(120+i*420)+'px';m.style.top='200px';m._boardTimerSetState({total:300,left:skin==='timer-liquid'?150:300,running:false});}});
