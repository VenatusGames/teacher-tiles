...
(async()=>{
  ...
  try{
    ...
    const retained=await page.evaluate(async()=>{const original=[...document.querySelectorAll('.sticker-picker-pack:not(.sticker-picker-most-used)')];window.dispatchEvent(new CustomEvent('teachertiles:stickerplaced',{detail:{key:document.querySelector('.sticker-picker .sticker-shelf-item').dataset.stickerSrc}}));await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(()=>requestAnimationFrame(r))));return original.every(el=>el.isConnected)});assert(retained,'placing reuses every unchanged pack');
    ...
    await page.mouse.move(1400,900);await page.waitForTimeout(450);assert.equal(await page.locator('.workspace .sticker-module>.module-delete').first().evaluate(el=>getComputedStyle(el).opacity),'0');
    ...
    const anchored=page.locator('.workspace .sticker-module').last();await anchored.hover();const rotateBefore=await anchored.locator('.sticker-rotate-handle').boundingBox(),deleteBefore=await anchored.locator('.module-delete').boundingBox();
    await anchored.evaluate(el=>{el.dataset.stickerRotation='65';el.style.setProperty('--sticker-rotation','65deg')});
    const rotateAfter=await anchored.locator('.sticker-rotate-handle').boundingBox(),deleteAfter=await anchored.locator('.module-delete').boundingBox();
    for(const [a,b] of [[rotateBefore,rotateAfter],[deleteBefore,deleteAfter]]){assert(Math.abs(a.x-b.x)<2&&Math.abs(a.y-b.y)<2,'handles stay anchored during rotation')}
    await anchored.evaluate(el=>{el.dataset.stickerRotation='0';el.style.setProperty('--sticker-rotation','0deg')});
    ...
    await page.evaluate(()=>{createStickerModule({emoji:'⭐'},900,400);document.body.classList.add('is-board-idle');document.querySelectorAll('.sticker-module').forEach(el=>el.classList.add('is-idle-unhovered'))});await page.mouse.move(1400,900);await page.waitForTimeout(450);assert.equal(await page.locator('.sticker-module>.module-delete').last().evaluate(el=>getComputedStyle(el).opacity),'0');
    await page.evaluate(()=>{document.body.classList.remove('is-board-idle');window.dispatchEvent(new CustomEvent('teachertiles:accountchange'));syncCosmeticEntitlements()});
    await page.locator('#theme-shelf-toggle').evaluate(el=>el.click());await page.waitForTimeout(400);
    await page.locator('.theme-picker-pack-heading').filter({hasText:'Pastel'}).click();assert.equal(await page.locator('#pastel-theme-fan>.subscription-access-crown').count(),0);
    assert.equal(await page.locator('.theme-picker [data-theme-choice^=pastel-]>.subscription-access-crown').count(),await page.locator('.theme-picker [data-theme-choice^=pastel-]').count());
    assert.equal(await page.locator('.theme-picker [data-theme-choice^=pastel-] .subscription-access-crown').first().evaluate(el=>el.offsetParent.classList.contains('theme-card')),true);
