...
(async()=>{
  ...
  try{
    ...
    assert(small.scale<.65&&small.scale>.4,JSON.stringify(small));assert.equal(small.width,300);assert.equal(small.height,430);assert.equal(small.scale,small.saved);
