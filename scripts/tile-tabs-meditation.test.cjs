...
(async()=>{
  ...
  try{
    ...
    assert.equal(await page.evaluate(()=>tileUniformScale(document.getElementById('resize-test'))),1,'first drag stops at boundary');
    await page.clock.runFor(1100);
    box=await page.locator('#resize-test').boundingBox();handle=await page.locator('#resize-test [data-resize=br]').boundingBox();
    await page.mouse.move(handle.x+handle.width/2,handle.y+handle.height/2);await page.mouse.down();await page.mouse.move(handle.x+handle.width/2-box.width*.45,handle.y+handle.height/2-box.height*.45,{steps:10});await page.mouse.up();
