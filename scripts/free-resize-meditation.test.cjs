...
(async()=>{
  ...
  try{
    ...
    await drag('b',0,-300);let g=await tile.evaluate(m=>[m.offsetWidth,m.offsetHeight,tileUniformScale(m)]);assert.deepEqual(g,[600,200,1]);
    await drag('r',-1000,0);g=await tile.evaluate(m=>[m.offsetWidth,m.offsetHeight,tileUniformScale(m)]);assert.deepEqual(g,[160,200,1]);
    await drag('b',0,1000);g=await tile.evaluate(m=>[m.offsetWidth,m.offsetHeight]);assert.deepEqual(g,[160,480]);
    await page.evaluate(()=>undoBoardAction());assert.equal(await tile.evaluate(m=>m.offsetHeight),200);
    ...
    await page.evaluate(()=>{const saved=TeacherTilesBoard.capture();TeacherTilesBoard.load(saved);});assert.equal(await tile.getAttribute('data-med-palette'),'dusk');assert.equal(await tile.evaluate(m=>m.offsetWidth),160);
    ...
    assert.deepEqual(await tile.evaluate(m=>[m.offsetWidth,m.offsetHeight,tileUniformScale(m)]),[300,200,1],'legacy scaled saves retain visible size');
