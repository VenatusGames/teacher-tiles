...
// These deterministic, local SVGs supply boards and the offline preview renderer.
...
// Preview images are baked from these sources by generate-theme-previews.cjs.
...
for (const [family, variants] of Object.entries(families)) {
  variants.forEach((variant, index) => {
    ...
    css += `body.theme-${theme}{--material-art:url("assets/themes/${theme}.svg")}\n`;
    css += `.theme-card--${card},\n.theme-pack__sheet--${card},\n.board-preview-theme-${theme}${shop}{--material-preview:url("assets/themes/previews/${theme}.webp")}\n`;
    if (family === 'cosmos') {
      css += `body.theme-${theme}{--cosmos-board-art:url("assets/themes/${theme}-board.svg")}\n`;
      css += `.board-preview-theme-${theme}{--cosmos-board-preview:url("assets/themes/previews/${theme}-board.webp")}\n`;
    }
  });
}
