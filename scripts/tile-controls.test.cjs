...
(async()=>{
  ...
  try{
    ...
    assert.equal(await opacity('.module-tab-add'),'1','site preference reveals add tab');
    ...
    assert.equal(await opacity('.meditation-heading'),'0');assert.equal(await opacity('.meditation-session'),'0');
    ...
    assert(compact[0]>500,'compact preserves wide shape when height reaches its limit first');assert.equal(compact[1],387);assert.equal(compact[2],.5);
