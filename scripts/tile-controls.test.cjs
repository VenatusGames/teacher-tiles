...
(async()=>{
  ...
  try{
    ...
    assert.equal(compact[0],160);assert.equal(compact[1],194);assert.equal(compact[2],1);
    ...
    await page.evaluate(()=>undoBoardAction());assert.equal(await tile.evaluate(m=>m.offsetWidth),160);
