...
const cursors=await page.evaluate(()=>{localStorage.setItem('teacherTilesOwnedShopPacks',JSON.stringify(TeacherTilesCursorPacks.map(p=>p.productId)));return TeacherTilesCursorPacks.flatMap(p=>p.cursors).map(c=>{applyAppCursor(c.id);return document.body.dataset.appCursor===c.id})});assert.equal(cursors.length,20);assert(cursors.every(Boolean));
...
assert.deepEqual(errors,[]);console.log('Flashcards: all five decks navigate, go back and restore position. Coin Flip: both outcomes and persistence. Cursors: all 20 variants selectable.');
